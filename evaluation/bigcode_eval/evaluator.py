...
class Evaluator:
    ...
    def evaluate(self, task_name, intermediate_generations=None, adapter_tasks=None):
        task = tasks.get_task(task_name, self.args)
        ...
        if self.accelerator.is_main_process:
            if not self.args.load_generations_path:
                save_generations_path = f"{os.path.splitext(self.args.save_generations_path)[0]}_{task_name}_{adapter_tasks}.json"
                self.save_json_files(generations, references, save_generations_path, f"references_{task_name}_{adapter_tasks}.json")
            ...
            results = task.process_results(generations, references, adapter_tasks)
            return results
